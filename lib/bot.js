		...
		bot.client.on("connected", function (server, port) {
		    ...
		    sync.lazy(function () {
				...
				if (typeof options.output.listUsers == "string") {
					...
					util.chatter.log(null, styles.console.info(options.output.listUsers.weaveStrict(userlist.length == 0 ? options.output.noUsers : userlist.join(options.output.list.separator))));
				}
