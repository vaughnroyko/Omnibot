...
(function () {
    ...
    process.exit = function (code) {
        // closing code should go in here
        exit(code);
    };
    ...
    process.on('uncaughtException', function (err) {
        try {
            if (typeof logger == "object") logger.withTimestamp(() => logger.logTo("error.log", err.stack), false);
        } catch (_e) {
...
if ([options.twitch.identity.username, options.twitch.identity.password, options.twitch.channel].indexOf("") > -1)
...
CommandManager = require("./core/commands.js"),
commands = bot.commands = new CommandManager(bot),
Client = require("./core/client.js"),
client = bot.client = new Client(options.twitch);

client.events = {
    chat: function (__, userData, message) {
        logger.log(userData['display-name'] + ": " + message);
        if (message[0] == "!") {
            // TODO @stats -> command
            var result = commands.call(message.slice(1));
            if (!result.success) {
...
Chatters = require("./core/chatters.js"),
chatters = new Chatters(bot);
