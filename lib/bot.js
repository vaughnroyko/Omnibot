...
try {
	...
	global.modules.fs.unlinkSync("error.log");
	...
	mongoose.connect(options.database.path, function(err) {
		if (err) {
			...
			util.log(null, styles.console.info('Connection to MongoDB successful.'));
			...
			bot.client.connect().then(function(status) {
				if (status) {
					util.log(null, styles.console.info("Connection to Twitch successful."));
					...
					bot.client.addListener('chat', function (channel, user, message) {
						...
						if (message[0] == options.core.commandPrefix) {
							if (options.output.showCommandCalls) util.log(user.name, styles.event.command(message));
					...
					util.log(null, styles.console.error("I couldn't connect to twitch. =("));
				}
			});
		}
	});
