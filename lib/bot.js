...
mongoose.connect(options.mongo.path, function(err) {
	if (err) {
		...
		global.bot = bot = {
			exit: function () {
				if (this.client) this.client.disconnect(); //Stop IRC
		...
		bot.client.on("connected", function (server, port) {
		    ...
		    sync.lazy(function () {
				...
				if (typeof options.output.listUsers == "string") {
					...
					util.chatter.log(null, styles.console.info(options.output.listUsers.format(userlist.length == 0 ? options.output.noUsers : userlist.join(options.output.list.separator))));
				}
