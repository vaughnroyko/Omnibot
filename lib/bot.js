...
var styles = global.styles = createStyles(options.styles);
...
mongoose.connect(global.options.database.path, function(err) {
	if (err) {
		...
		global.bot = bot = {
			...
			commands: require('./command/core.js')
		};
				...
				bot.client.addListener('chat', function (channel, user, message) {
					...
					if (message[0] == options.core.commandPrefix) {
						...
						if (message[0].substring(1) in bot.commands) {
							...
							console.log(user.name + " tried to run the command with the name " + message[0].substring(1) + " but that command doesn't exist!")
