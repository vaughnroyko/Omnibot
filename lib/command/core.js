    ...
    */
    uptime: function () {
        if (util.isLive()) {
            ...
            util.chatter.say(options.output.streamTime.weaveStrict(bot.channel, hours, minutes));
        } else {
            util.chatter.say(options.output.notLive.weaveStrict(bot.channel));
        }
    },
    /*
    ...
    */
    time: function (user, who) {
        ...
        if (util.viewers.isBlacklisted(user)) {
            util.chatter.say(options.output.core.time.blacklisted.weaveStrict(
            ...
            if (user && "time" in user) {
                util.chatter.say(options.output.core.time.normal.weaveStrict(
                    user.name,
                    user.time,
                    self
                ));
            } else {
                util.chatter.say(options.output.core.time.unknown.weaveStrict(
                    user.name,
                    self
                ));
            }
