...
addCommands({
    ...
    */
    uptime: function () {
        if (util.isLive()) {
            ...
            util.chatter.say(options.output.bot.time.weave(global.bot.channel, hours, minutes));
        } else {
            util.chatter.say(options.output.bot.notLive.weave(global.bot.channel));
        }
    },
    /*
    ...
    */
    stop: function (user) {
        if (user.rank >= ranks.admin) {
            util.chatter.say(options.output.bot.stop.weave(global.bot.name));
            global.bot.exit();
        }
    },
    /*
    ...
    */
    restart: function (user) {
        if (user.rank >= ranks.admin) {
            util.chatter.say(options.output.bot.restart.weave(global.bot.name));
            global.bot.exit(4);
        }
    },
    /*
    ...
    */
    whois: function (user, who) {
        ...
        var out = options.output.core.whois.weave(user, util.viewers.stats.rank(user), (function () {
            var result = "";
    ...
    */
    rank: function (user, who, rank) {
        try {
            ...
            util.chatter.say(options.output.core.rank.notExist.weave(who));
            throw err;
        }
        if (user.rank >= ranks.admin && user.rank > who.rank) {
            ...
            if (rank < ranks.broadcaster) {
                ...
                util.chatter.say(options.output.core.rank.success.weave(who, rankName));
                return;
            }
        }
        util.chatter.say(options.output.core.rank.invalid.weave(rank));
    }
});
