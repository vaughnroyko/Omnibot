...
addCommands({
    ...
    */
    uptime: function () {
        if (util.isLive()) {
            var started = new Date(bot.data.stream.created_at);
            var now = new Date();
            var uptime = now - started;
            var hours = Math.floor((uptime % 86400000) / 3600000);
            var minutes = Math.floor(((uptime % 86400000) % 3600000) / 60000);
            util.chatter.say(options.output.bot.time.weave(bot.channel, hours, minutes));
        } else {
            util.chatter.say(options.output.bot.notLive.weave(bot.channel));
        }
    },
    /*
    ...
    */
    stop: function (user) {
        if (user.rank >= ranks.admin) {
            util.chatter.say(options.output.bot.stop.weave(bot.name));
            bot.exit();
        }
    },
    /*
    ...
    */
    restart: function (user) {
        if (user.rank >= ranks.admin) {
            util.chatter.say(options.output.bot.restart.weave(bot.name));
            bot.exit();
        }
    },
    /*
