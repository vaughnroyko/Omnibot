    ...
    */
    stop: function (user, params) {
        if (user.rank >= ranks.Broadcaster || (options.core.modsCanRunBroadcasterCommands && user.rank == ranks.Moderator)) {
            ...
            util.log(null, styles.console.info(user.name + " stopped the bot!"));
            bot.exit();
        }
    },
    /*
    ...
    */
    time: function (user, params) {
        ...
        if (user.rank == ranks.Broadcaster || user.name == global.options.twitch.identity.username) {
            util.say((self? "Your" : (user.name == options.twitch.identity.username ? "My own": user.name + "'s")) + " amount of time logged exceeds the limits of my computational power" + (self? ", " + user.name : "") + ".");
        } else {
            global.database.users.findOne({username: user.name}, function(err, docs) {
                if (docs) {
                    util.say((self? "You have " : user.name + " has ") + docs.time + " minutes logged" + (self? ", " + user.name : "") + "!");
                } else {
