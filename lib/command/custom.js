        ...
        database.commands.findOne({name: name}, function (err, command) {
            if (command) {
                ...
                if (options.output.commands.logCommandFails) {
                    util.chatter.log(null, styles.console.error(options.output.commands.notFound.weave(name, user)));
                }
            ...
            if (action == "add" || action == "create") {
                if (customCommands.names.indexOf(name) < 0 && !(name in bot.commands)) {
                    ...
                    command.save(function(err, result) {
                        if (err) {
                            util.chatter.say(options.output.commands.create.fail.weave(name));
                        } else {
                            customCommands.load(name);
                            util.chatter.say(options.output.commands.create.success.weave(name));
                        }
                    });
                } else {
                    util.chatter.say(options.output.commands.create.exist.weave(name));
                }
                ...
                if (index > -1) {
                    database.commands.remove({name: name}, function (err, result) {
                        if (err) {
                            util.chatter.say(options.output.commands.remove.fail.weave(name));
                        } else {
                            customCommands.names.splice(index, 1);
                            delete bot.commands[name];
                            util.chatter.say(options.output.commands.remove.success.weave(name));
                        }
                    });
                } else {
                    util.chatter.say(options.output.commands.notExist.weave(name));
                }
            } else if (action == "edit") {
                var index = customCommands.names.indexOf(name);
                if (index > -1) {
                    ...
                    database.commands.update(
                        ...
                        function (err, result) {
                            if (err) {
                                util.chatter.say(options.output.commands.edit.fail.weave(name));
                            } else {
                                util.chatter.say(options.output.commands.edit.success.weave(name));
                            }
                        }
                    );
                } else {
                ...
                if (index > -1) {
                    database.commands.update(
                        ...
                        function (err, result) {
                            if (err) {
                                util.chatter.say(options.output.commands.rename.fail.weave(name));
                            } else {
                                customCommands.names.splice(index, 1);
                                delete bot.commands[name];
                                customCommands.load(extra);
                                util.chatter.say(options.output.commands.rename.success.weave(name, extra));
                            }
                        }
                    );
                } else {
