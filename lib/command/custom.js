        ...
        database.commands.findOne({name: name}, function (err, command) {
            if (command) {
                ...
                if (options.output.commands.logCommandFails) {
                    util.chatter.log(null, styles.console.error(options.output.commands.notFound.weave(name)));
                }
            ...
            if (action == "add" || action == "create") {
                if (customCommands.names.indexOf(name) < 0) {
                    ...
                    command.save(function(err, result) {
                        if (err) {
                            util.chatter.say(options.output.create.fail.weave(name));
                        } else {
                            global.customCommands.load(name);
                            util.chatter.say(options.output.create.success.weave(name));
                        }
                    });
                } else {
                    util.chatter.say(options.output.create.exist.weave(name));
                }
                ...
                if (index > -1) {
                    database.commands.remove({name: name}, function (err, result) {
                        if (err) {
                            util.chatter.say(options.output.remove.fail.weave(name));
                        } else {
                            global.customCommands.names.splice(index, 1);
                            delete global.bot.commands[name];
                            util.chatter.say(options.output.remove.success.weave(name));
                        }
                    });
                } else {
                    util.chatter.say(options.output.notExist.weave(name));
                }
            } else if (action == "edit") {
                var index = customCommands.names.indexOf(name);
                if (index > -1) {
                    ...
                    database.commands.update(
                        ...
                        function (err, result) {
                            if (err) {
                                util.chatter.say(options.output.edit.fail.weave(name));
                            } else {
                                util.chatter.say(options.output.edit.success.weave(name));
                            }
                        }
                    );
                } else {
                ...
                if (index > -1) {
                    database.commands.update(
                        ...
                        function (err, result) {
                            if (err) {
                                util.chatter.say(options.output.rename.fail.weave(name));
                            } else {
                                global.customCommands.names.splice(index, 1);
                                delete global.bot.commands[name];
                                customCommands.load(extra);
                                util.chatter.say(options.output.rename.success.weave(name, extra));
                            }
                        }
                    );
                } else {
