...
var addPointsForUser = function (user) {
    ...
    newPoints.save(function(err, result) {
        chatter.print(styles.points(options.output.points.firstjoin.weave(user.name, options.core.modules.points.startWith)));
    });
};
...
var outputViewerPoints = function (user) {
    database.points.findOne({user: user.name}, function(err, points) {
        if (points) {
            chatter.print(styles.points(options.output.points.joinInfo.weave(user.name, points.count)));
        } else {
