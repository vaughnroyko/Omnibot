...
var addPointsForUser = function (user) {
    ...
    newPoints.save(function(err, result) {
        chatter.print(styles.points(options.output.points.firstjoin.weave(user, options.core.modules.points.startWith)));
    });
};
    ...
    donate: function (user, amount, to) {
        ...
        if (points.count >= amount) {
            ...
            if (to && toPoints) {
                 ...
                 chatter.say(options.output.points.donate.success.weave(user, to, amount));
            } else {
                chatter.say(options.output.points.donate.notExist.weave(user.name, to));
            }
        } else {
            chatter.say(options.output.points.donate.notEnough.weave(user, amount - points.count));
        }
    },
