...
var addPointsForUser = function (user) {
    ...
    newPoints.save(function(err, result) {
        chatter.log(null, styles.points(options.output.points.firstjoin.weave(user.name, options.core.modules.points.startWith)));
    });
};
...
var outputViewerPoints = function (user) {
    database.points.findOne({user: user.name}, function(err, points) {
        if (points) {
            chatter.log(null, styles.points(options.output.points.joinInfo.weave(user.name, points.count)));
        } else {
    ...
    donate: function (user, amount, to) {
        ...
        if (points.count >= amount) {
            ...
            if (toPoints) {
                 ...
                 chatter.say(options.output.points.donate.success.weave(user.name, to, amount));
            } else {
                chatter.say(options.output.points.donate.notExist.weave(user.name, to));
            }
        } else {
            chatter.say(options.output.points.donate.notEnough.weave(user.name, amount - points.count));
        }
    },
    ...
    */
    top: function (user, count) {
        ...
        database.points.find({
            ...
            if (points) {
                chatter.say(options.output.points.top.weave(points.length, points));
            } else {
    ...
    */
    balance: function (user, who) {
        ...
        if (util.viewers.isBlacklisted(user)) {
            chatter.say(options.output.points.balance.blacklisted.weave(
            ...
            if (typeof points == "number") {
                chatter.say(options.output.points.balance.normal.weave(
                    user,
                    points,
                    self
                ));
            } else {
                chatter.say(options.output.points.balance.unknown.weave(
                    user,
                    self
                ));
            }
