    ...
    */
    balance: function (user, who) {
        ...
        if (util.viewers.isBlacklisted(user)) {
            util.chatter.say(options.output.points.balance.blacklisted.weaveStrict(
            ...
            global.database.points.findOne({user: user.name}, function(err, points) {
                if (points && points.count) {
                    util.chatter.say(options.output.points.balance.normal.weaveStrict(
                        user.name,
                        points.count,
                        self
                    ));
                } else {
                    util.chatter.say(options.output.points.balance.unknown.weaveStrict(
                        user.name,
                        self
                    ));
                }
            ...
            if (word == "from") {
                ...
                var update = function () {
                    global.database.users.find({
                        ...
                        { multi: true },
                        function (err, raw) {
                            util.chatter.say(styles.points(message.weaveStrict(
