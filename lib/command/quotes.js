...
database.quotes = global.modules.mongoose.model(global.options.twitch.channels[0] + '.quotes', global.modules.mongoose.Schema({
    quote: String,
    username: String,
    added: Date
}));
    ...
    */
    recite: function (user, params) {
        ...
        database.quotes.find().limit(-1).skip(rand).next().toArray().map(function(doc) {
            util.chatter.say(doc.name + ": " + doc.quote);
        });
    }
}