    ...
    _.extend(module.exports, {
        ...
        */
        ...
        /*
          ...
          limitations: broadcaster || (moderator && modsCanRunBroadcasterCommands [defaults to true in core.cson])
        */
        steal: function (user, params, message) {
            if (!message) message = options.output.stealSuccess;
            if (user.rank >= ranks.Broadcaster || (options.core.modsCanRunBroadcasterCommands && user.rank == ranks.Moderator)) {
                if (params[1] == "from") {
                    var bounty = params[0];
                    if (bounty == "*") {
                        ...
                        global.bot.database.users.find({
                            ...
                            function (err, raw) {
                                util.say(styles.points(message.format(
                                    (prior === null ? params[0] * raw.nModified : (raw.nModified == 0 ? 0 : prior - raw.nModified * options.core.points.startWith)),
                                    raw.nModified
                                )));
                            }
                        );
                    ...
                    var prior = null;
                    if (params[0] == "*") {
                        global.bot.database.users.aggregate(
                            {$match: {
                                $nor: [
                                    {username: global.bot.channel},
                                    {username: options.twitch.identity.username}
        ...
        distribute: function (user, params) {
            if (params[1] == "to") {
                ...
                this.steal(user, params, options.output.distributeSuccess);
            }
        }
    });
}