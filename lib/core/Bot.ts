...
export class Bot {
	...
	get isLive () { return this.channel.data.live; }
	...
	runCommand (user: Chatter, input: string) {
		// TODO @stats -> command
		const result = this.commands.call(input, user);
		if (!result.success) {
	...
	connect (connectionOptions: { username: string, password: string }) {
		...
		try {
			this.database = new Database(this.options.mongo.path + "V" + databaseVersion + "#" + this.channel.data.name);
		} catch (err) {
			// if we can't connect to the database, there's no point in going any farther
			this.logger.timestamp = false;
			const errorMessage = err.name == "MongoError" && err.message.startsWith("connect ECONNREFUSED") ?
				"{#red:Couldn't connect to MongoDB.}" : "{#red:{name}: {message}\n{1}}";
			this.logger.log(errorMessage.weave(err, Weaving.Error.trim(err.stack)));
			this.logger.log("Press any key to exit.");
			Console.input.onChar(() => {
				this.stop();
			});
			return;
		}
		...
		// connect to twitch
		this.chat = new Chat(this.database, () => this.channel.data.live, this.options);
		...
		this.plugins = new PluginManager("plugins", this.chat, this.commands, this.database, this.channel.data, this.options);
		...
		this.chat.onWhisper = (user: Chatter, message: string, isReceived: boolean) => {
			const whisper = output.messages.whisper;
		...
		this.logger.log(output.bot.connecting.weave({ channel: this.channel.data }));
		this.chat.connect({
			channel: this.channel.data.name,
			identity: connectionOptions,
		});
		this.logger.log(output.bot.connected.weave({ channel: this.channel.data }));

		// enable input from the console now
		Console.input.enable();
		Console.input.onLine((message: string) => {
			if (message[0] == "!") {
