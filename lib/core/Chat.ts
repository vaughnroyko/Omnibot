...
export class Chat {
	...
	public isBlackListed (chatter: Chatter) {
		return chatter.rank >= Ranks.broadcaster || this.options.core.blacklist.includes(chatter.name);
	}

	constructor(database: Database, isLive: () => boolean, private options: Options) {
		...
		this.client.on("whisper", (userData: UserData, message: string, self: boolean) => {
			if (!self) {
				const chatter = this.getChatter(userData);
	...
	getChatter (data: string | UserData): Chatter {
		...
		let result = this.chatters.where({ name }).findOne();
	...
	private join (chatter: string | UserData) {
		const ch = this.getChatter(chatter);
		ch.chatting = true;
		ch.save();
	...
	whisper (to: Chatter | string, ...what: any[]) {
		const user: string = typeof to == "string" ? to as string : (to as Chatter).name;
