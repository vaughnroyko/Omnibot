...
export module Library {
	...
	export function remove (lib: Library, ...toRemove: Library[]) {
		for (const libToRemoveWith of toRemove) {
			for (const commandName in libToRemoveWith) {
...
export class Commands {
	...
	private match (expectedArguments: Argument[], givenArguments: string[]): any[] | ArgumentsProvidedError {
		const result: any[] = [];
		let consumeRemaining: Argument, optionalRemaining = false;
		for (let i = 0; i < expectedArguments.length || (consumeRemaining && i < givenArguments.length); i++) {

			const given = givenArguments[i];
			let expected = consumeRemaining || expectedArguments[i];
	...
	call (input: string, chatter: Chatter): { success: boolean, result?: any } {
		const result: { success: boolean, result?: any } = { success: false };
		const splitCommand = input.split(/\s+/);
		...
		const command = lib[name] as Command;
		let args: any[] = [];
		if ("args" in command) {
		...
		if ("rank" in command) {
			const rank = command.rank;
			if (typeof rank == "object") {
				const rankMatcher = rank as RankMatcher;
				if (
