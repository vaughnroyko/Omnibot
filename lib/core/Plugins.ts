...
import { IChannelData } from "./interfaces/IChannel";

import { Error as WeavingError } from "weaving";
...
class PluginError extends WeavingError {
	name = "PluginError";
	weavingMessage = "Could not load the plugin '{0}'{1?~: {1}}";
	proto = true;
}
class InvalidPluginDefinitionFileError extends WeavingError {
	name = "InvalidPluginDefinitionFileError";
...
export class PluginWrapper {
	...
	constructor(directoryOrPlugin: string | Plugin, private api: PluginAPI) {
		if (typeof directoryOrPlugin == "string") {
			const directory = this.directory = directoryOrPlugin;
...
export class PluginManager {
	...
	constructor(directory: string, chat: Chat, commands: Commands, database: Database, channel: IChannelData, options: Options) {
	...
	add (...plugins: (Plugin | string)[]) {
		for (const pluginOrDirectory of plugins) {
			try {
				const plugin = new PluginWrapper(pluginOrDirectory as any, this.api);
	...
	forEach (callback: (plugin: PluginWrapper) => any) {
		for (const plugin of this.plugins) callback(plugin);
	}

	// events
	onInit () {
		for (const wrapper of this.plugins) if (wrapper.plugin.onInit) {
			wrapper.plugin.onInit();
		}
	}
	onUnknownCommand (commandName: string): Command | Library {
		for (const wrapper of this.plugins) {
			if (wrapper.plugin.onUnknownCommand) {
				const result = wrapper.plugin.onUnknownCommand(commandName);
				if (result) return result;
			}
		}
	}
	onChatterJoin (chatter: Chatter, isNew: boolean) {
		for (const wrapper of this.plugins) if (wrapper.plugin.onChatterJoin) {
			wrapper.plugin.onChatterJoin(chatter, isNew);
		}
	}
	onChatterPart (chatter: Chatter, isNew: boolean) {
		for (const wrapper of this.plugins) if (wrapper.plugin.onChatterPart) {
			wrapper.plugin.onChatterPart(chatter, isNew);
		}
	}
	onUpdate () {
		for (const wrapper of this.plugins) if (wrapper.plugin.onUpdate) {
