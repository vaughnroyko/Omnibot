...
var chatters = require("./chatters.js");
...
var commands = module.exports = function (bot) {
    ...
    return {
        ...
        call: function (command, chatter) {
            ...
            logger.log("attempting to call ", name, args);
            if (name in library) {
                ...
                if ("rank" in command) {
                    ...
                    if (typeof rank == "object") {
                        if (
                            ...
                            ("max" in rank && chatter.rank > chatters.getRank(rank.min))
                        ) return false;
                    } else {
                        if (chatter.rank < chatters.getRank(rank.min)) return false;
                    }
                }
                command.call(args);
            }
