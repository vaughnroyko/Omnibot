...
global.database = {
	...
	quotes: mongoose.model(global.options.twitch.channels[0] + '.quotes', mongoose.Schema({
		quote: String,
		username: String,
		added: Date
	})),
	commands: mongoose.model(global.options.twitch.channels[0] + '.commands', mongoose.Schema({
		name: String,
		rank: Number,
		output: String
	}))
