...
if (fs.existsSync("logs/new/bot.log")) {
    var t = new Date(fs.statSync("logs/new/bot.log").mtime);
	...
	fs.renameSync("logs/new/bot.log", currentFolder + "/" + formatTime(formats.time) + ".log");
}
fs.writeFileSync("logs/new/debug.log", "");
...
module.exports = chatter = {
    ...
    log: function (user) {
        ...
        fs.appendFileSync("logs/new/bot.log", chalk.stripColor(message + "\n"));
        console.log(message);
    },
...
chatter.log.debug = function (message) {
	fs.appendFileSync("logs/new/debug.log", chalk.stripColor(message + "\n"));
};