...
var viewerHandler = {
                ...
                if (typeof savedRank == "number" && savedRank >= 0) {
                    if (ircRank < savedRank || ircRank > Math.floor(savedRank / 2) * 2) {
                        ...
                        if (savedRank == ranks.new && user.time >= options.core.userTrustTime) {
                            ...
                            util.chatter.log(null, options.output.trustUser.format(user.name));
                        }
        ...
        request('https://tmi.twitch.tv/group/user/' + global.bot.channel + '/chatters', function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                for (var username in chatters) {
                    ...
                    if (!u || !u.chatting) {
                        viewerHandler.join(username, chatters[username]);
                    } else {
    ...
    join: function (username, callback) {
        ...
        if (!user || !user.chatting) {
            ...
            var skip = false;
            if (username == global.bot.channel)
                rank = ranks.broadcaster, skip = true;
            if (username == options.twitch.identity.username)
            ...
            if (skip) {
                ...
                var u = viewerHandler.updateUser(user, {rank: rank});
                if (u) {
                    user = u;
                    ...
                    viewerHandler.create(user, callback);
                }
            }
        }
    },
