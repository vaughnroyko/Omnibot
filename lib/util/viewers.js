...
var updateUserDoc = function (user, data) {
...
var updateDisplayName = function (name) {
    request('https://api.twitch.tv/kraken/users/' + name, function (err, response, body) {
        if (!err && response.statusCode == 200) {
            response = JSON.parse(body);
            updateUserDoc(name, {displayName: response.display_name});
        } else {
