    ...
    cleanup: function () {
        ...
        for (var i = 0; i < users.length; i++) {
            if (!users[i].name || handler.isBlacklisted(users[i])) {
                ...
                if (users[i].username) uu(users[i], {name: users[i].username, username: undefined});
                if (users[i].firstjoin) uu(users[i], {create: users[i].firstjoin, firstjoin: undefined});
                if (!("rank" in users[i])) uu(users[i], {rank: 0});
            }
        }
        this.updateTrust();
    },
